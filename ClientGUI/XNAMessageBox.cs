...
namespace ClientGUI
{
    ...
    public class XNAMessageBox : XNAWindow
    {
        ...
        public XNAMessageBox(WindowManager windowManager,
            string caption, string description, XNAMessageBoxButtons messageBoxButtons, bool isBlocking = false)
            : base(windowManager)
        {
            ...
            if (isBlocking)
            {
                dialogClosedEvent = new ManualResetEvent(false);
            }
        }

        private ManualResetEvent dialogClosedEvent;
        ...
        private void Hide()
        {
            ...
            dialogClosedEvent?.Set();
        }

        public void Show()
        {
            DarkeningPanel.AddAndInitializeWithControl(WindowManager, this);
            dialogClosedEvent?.WaitOne();
        }

        #region Static Show methods

        public static ManualResetEvent dialogClosedSEvent;
        ...
        public static void Show(WindowManager windowManager, string caption, string description,bool isBlocking = false)
        {
            ...
            if (isBlocking)
            {
                dialogClosedSEvent = new ManualResetEvent(false);
                dialogClosedSEvent.WaitOne();
            }

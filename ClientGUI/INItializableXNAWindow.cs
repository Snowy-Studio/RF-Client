...
namespace ClientGUI
{
    public class INItializableXNAWindow : XNAWindow
    {
        public INItializableXNAWindow(WindowManager windowManager) : base(windowManager)
        {
        }
        ...
        protected void ReadINIForControl(XNAControl control)
        {
            ...
            foreach (var kvp in section.Keys)
            {
                ...
                else if (kvp.Key == "$Text")
                {
                    ...
                    control.Text = section.GetStringValue(nameof(control.Text), string.Empty).L10N($"UI:Main:{kvp.Value}");
                   
                }
                else
                {
                    control.ParseControlINIAttribute(ConfigIni, kvp.Key, kvp.Value);
                }

...
namespace Ra2Client.DXGUI.Multiplayer.GameLobby
{
    ...
    public abstract class MultiplayerGameLobby : GameLobbyBase, ISwitchable
    {
        ...
        protected override void BtnLaunchGame_LeftClick(object sender, EventArgs e)
        {
            ...
            if (Map.MaxPlayers < 1)
            {
                AddNotice("There is a suspected problem with this map, and the client does not think there is a player spawn point.".L10N("UI:Main:MapError"));
                return;
            }
        ...
        protected override void WriteSpawnIniAdditions(IniFile iniFile)
        {
            ...
            iniFile.SetValue("Settings", "FrameSendRate", FrameSendRate);
            if (MaxAhead > 0)
                iniFile.SetValue("Settings", "MaxAhead", MaxAhead);
            iniFile.SetValue("Settings", "Protocol", ProtocolVersion);
        }

        protected override int GetDefaultMapRankIndex(GameModeMap gameModeMap)
        {
            if (gameModeMap.Map == null) return -1;

            if ((gameModeMap.Map?.MaxPlayers ?? 0) > 3)
